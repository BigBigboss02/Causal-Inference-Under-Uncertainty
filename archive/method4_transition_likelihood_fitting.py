...
TARGET_FOLDER = SWEEP_ROOT / r"theta_1_1__gen_0p1__trueprior_0p01__particles_30__runs_100__trials_70"

KID_JSON_PATH = Path(r"data\Dolly_KeyEviModel_7.3.24.json")

saving_summary_name_86 = "sweep_transition_likelihood_fit_summary_vs3.json"
...
def compute_child_loglik(
    ...
    # t = 1 → no transition, define loglik = 0
    trial_details.append({
        "trial": 1,
        "n_prev": None,
        "n_curr": int(child_counts[0]),
    ...
    for t in range(2, max_trials + 1):
        ...
        n_prev = int(child_counts[t - 2])   # K_{t-1}
        n_curr = int(child_counts[t - 1])   # K_t

        p_prev = float(dist[t - 1][n_prev])
        p_curr = float(dist[t][n_curr])

        # Marta's transition approximation
        if p_prev > 0:
            trans_p = p_curr / p_prev
        else:
            trans_p = eps

        # ensure valid probability
        trans_p = max(trans_p, eps)

        logp = math.log(trans_p)
        total_loglik += logp
...
#     # First trial has no prior transition
#     trial_details.append(
#         {
#             "trial": 1,
#             "n_opened_child": int(child_counts[0]),
#             "assumed_prev_opened_for_transition": None,
#             "p_assumed_prev_n_given_tminus1_theta": None,
#             "p_curr_n_given_t_theta": float(dist[1][int(child_counts[0])]),
#             "approx_transition_prob": None,
#             "log_prob": 0.0,
#         }
#     )
...
#         # Marta-style "opened a box and reached its current state":
#         # use denominator based on n_curr - 1, not the child's actual previous state
#         assumed_prev = n_curr - 1
...
#             approx_trans_p = 0.0
#         else:
#             p_prev = float(dist[t - 1][assumed_prev])

#             if p_prev > 0:
#                 approx_trans_p = p_curr / p_prev
#             else:
#                 approx_trans_p = 0.0

#         # Marta-style thresholding:
#         # if valid positive ratio -> take log
#         # else -> add eps directly, NOT log(eps)
#         if approx_trans_p > 0:
#             logp = math.log(approx_trans_p)
#         else:
#             logp = eps
...
#             {
#                 "trial": t,
#                 "n_opened_child": n_curr,
#                 "assumed_prev_opened_for_transition": assumed_prev,
#                 "p_assumed_prev_n_given_tminus1_theta": p_prev,
#                 "p_curr_n_given_t_theta": p_curr,
#                 "approx_transition_prob": approx_trans_p,
#                 "log_prob": logp,
#             }
#         )
...
def process_one_folder(
    ...
    per_folder_out = folder_path / "transition_likelihood_fit_results.json"
...
def main() -> None:
    ...
    summary = {
        ...
        "max_trials": MAX_TRIALS,
        "eps": EPS,
        "record_type": "transition_likelihood_fitting_summary",
        ...
        "all_results_ranked_by_mean_loglik": ranked_by_mean,
        "failed_folders": failed_folders,
    }

    summary_path = SWEEP_ROOT / saving_summary_name_86
