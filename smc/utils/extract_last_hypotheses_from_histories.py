"""
Extract "last hypothesis" programs from saved run histories.

This mirrors the format used in gpt5.2 partially-observed logs:
- scan a directory for run JSON files
- for each run, pull the last non-null `hypothesis` from `history`
- write a summary JSON with per-run metadata + last hypothesis
"""
